  ...
  run :=
    withSetOptionIn fun stx =>
      Rule.runIfEnabled (α := α) stx fun m => do
        emitCheck (Check.node (α := α) m)
          (Check.severity (α := α))
          (Check.category (α := α))
          (Check.tags (α := α))
          (Rule.option (α := α)).name
          (Rule.message (α := α) m)
          (Check.explanation (α := α) m)
...
initialize _checkRuleAttr : TagAttribute ←
  registerTagAttribute `check_rule
    "Register a Check instance as a heron linter rule."
    (validate := unsafe checkRuleHandler)
