...
partial def syntaxToJson (stx : Syntax) : Json :=
  ...
  | .node _ kind args =>
    ...
    Json.mkObj [
      ("type", "node"),
      ("kind", shortKind),
      ("children", Json.arr (args.mapIdx fun i child =>
        Json.mkObj [("index", (i : Nat)), ("value", syntaxToJson child)]))]

def main (args : List String) : IO Unit := do
  let env ← mkEmptyEnvironment
  let input ← match args with
    | ["--input", input] => pure input
    | [filepath] => IO.FS.readFile filepath
    | [] => readStdin
    | _ => do
      IO.eprintln "Usage: dump-ast [--input <string> | <file> | stdin]"
      IO.Process.exit 1
  match runParserCategory env `command input with
  | .ok stx => IO.println (syntaxToJson stx).pretty
  | .error msg =>
    IO.eprintln s!"Parse error: {msg}"
    IO.Process.exit 1
where
  readStdin : IO String := do
